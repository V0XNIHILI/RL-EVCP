...
class PVDevice(Device):
    ...
    def update_timestep(self, t_str):
        ...
        p_generation = self.info['current_episode_data'][t_str] * self.power_rating
        assert p_generation <= 0, 'Device %s received positive value: %.2f' % (self, p_generation)
    ...
    def get_p_bounds(self, t_str, target_dt_min=None, uncertainty='deterministic'):
        ...
        if uncertainty == 'monthly scenarios':
            p_min = [np.mean([day_data[t_str] for t_str in t_str_list]) * self.power_rating
                     for day_data in self.info['current_episode_month_data']]
        elif uncertainty == 'monthly average':
            p_min = [sum([np.mean([day_data[t_str] for t_str in t_str_list]) for day_data
                          in self.info['current_episode_month_data']]) / len(self.info['current_episode_month_data'])
                     * self.power_rating]
        else:
            p_min = [np.mean([self.info['current_episode_data'][t_str] for t_str in t_str_list]) * self.power_rating]
        p_max = [0 for _ in range(len(p_min))]
        p_min = [min(0, p_min_i) for p_min_i in p_min]
        return p_min, p_max
